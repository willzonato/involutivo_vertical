...
```{r runif, fig.height=3, fig.width=4}
v <- runif(10^5)
hist(v, freq = FALSE)
curve(dunif(x),
...
```{r rnorm, fig.height=3, fig.width=4}
...
```{r rbeta1, fig.height=3, fig.width=4}
...
```{r rbeta2, fig.height=3, fig.width=4}
...
kable(S, 
      format = ifelse(type == "html", "markdown", type),
      caption = "Matriz de correlação desejada entre as variáveis aleatórias", 
      booktabs = TRUE) %>%
...
kable(t(WV), booktabs = TRUE, longtable = TRUE) %>% 
  kable_styling(latex_options = "striped", 
                bootstrap_options = "striped")
```
...
kable(FC(fcv_provavel, fci_provavel, bdi_i = bdi_i, cor = cor, tma = tma), 
      format = ifelse(type == "html", "markdown", type),
      digits = 2,
...
kable(t(WV_otimista),
      format = ifelse(type == "html", "markdown", type),
      caption = "Velocidade de Vendas -- Cenário Otimsta",
      booktabs = TRUE) 
```
