...
range_Pgv <- params$range_Pgv/100
names(range_Pgv) <- c("min", "max")
...
Pgv <- area_construida*valor_venda
...
O preço de venda praticado pelo mercado na região do imóvel é de `r reais(valor_venda)`/$m^2$, o que gera um Pgv de `r reais(Pgv)`. 
...
fcv_provavel <- FCV(vgv = Pgv, wv = wv)
kable(FC(fcv_provavel, fci_provavel, bdi_i = bdi_i, cor = cor, tma = tma), digits = 2,
...
Em relação às vendas, consideraremos uma variação entre `r 100*range_Pgv["min"]`\% e `r 100*range_Pgv["max"]`\% do Pgv provável.

```{r s_Pgv, echo = FALSE, message = FALSE}
s_Pgv <- sensibilidade_venda(range = range_Pgv,
                               cc = cc, wc = wc,
                               vgv = Pgv, wv = wv,
                               bdi_i = bdi_i, bdi_c = bdi_c,
                               cor = cor, tma = tma)
kable(s_Pgv, digits = 2,
      caption = "Sensibilidade do VPL à variação do Pgv", 
      format.args = list(big.mark = ".", decimal.mark = ",")
      )      
```
...
pander(t(WV_pessimista), split.table = 80) 
```
...
```{r s_plots, echo = FALSE, fig.width = 6, fig.height = 4.5}
join_tbl <-
  s_tma %>%
  full_join(s_custo) %>%
  full_join(s_Pgv) %>%
...
p3 <- ggplot(join_tbl, aes(x = Vendas/div, y = VPL/div)) + 
  ...
  scale_y_continuous(labels = gcomma) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Pgv (milhares de R$)", y = "VPL (R$ x 1.000)")
...
kable(FC_pessimista, caption = "Fluxo de Caixa pessimista do Empreendimento", 
      format.args = list(big.mark = ".", decimal.mark = ",")
      )
```
...
fcv_otimista <- FCV(range_Pgv["max"]*Pgv, w = wv_otimista)
...
kable(FC_otimista, caption = "Fluxo de Caixa otimista do Empreendimento", 
      format.args = list(big.mark = ".", decimal.mark = ",")
      )
```
...
```{r histogramasuniforme, echo=FALSE, fig.width = 6, fig.height = 8}
...
```{r variasbeta, echo = FALSE, fig.align = "center"}
...
```{r histogramasbeta, echo=FALSE, fig.width = 6, fig.height = 8}
...
kable(s, caption = "Estatísticas descritivas das diferentes simulações", 
      format.args = list(big.mark = ".", decimal.mark = ","))
```
...
kable(descr, caption = "Resumo das médias e desvios", 
      format.args = list(big.mark = ".", decimal.mark = ","))
```
